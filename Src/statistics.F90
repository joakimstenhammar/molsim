...
subroutine ScalarSampleExtrap(iStage, ilow, iupp, var)
   ...
   case (7)  ! after simulation
      ...
      do i = ilow, iupp
         nblocklen = max(1,min(int(1+log(float(max(var(i)%nsamp,1)))/log(2.0d0)),mnblocklen)) ! number of block lenghts !##FLAG: require at least one sample; otherwise the log will fail
